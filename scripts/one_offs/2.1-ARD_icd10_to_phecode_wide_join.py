...
WIDE JOIN: means that for an ICD10 code X00, 
phecode mapping will be done such that any phecode mapped to X00.XX
will be mapped in a separate row. a column listing the specfiic phecode
and icd10 code (xx.xx) will be included.
...
#make new column for ICD10 category only in phecode_map
phecode_map['ICD10_category'] = phecode_map['ICD10'].str[:3] 


#drop duplicates in ard_icd10 before merge
ARD_icd10 = ARD_icd10.drop_duplicates()

#drop duplicates in ard_icd10 in icd10_explo column
ARD_icd10 = ARD_icd10.drop_duplicates(subset=['ICD10_explo'])
...
mapped_success = mapped[mapped['ICDO10_Phecode_mapped_flag']]
mapped_failure = mapped[~mapped['ICDO10_Phecode_mapped_flag']]
...
# Save outputs
# ==============================
mapped.to_csv('data/2-phecode_mapping/ARD_GBD_ICD10_phecode_mapped_WIDE.csv', index=False)
